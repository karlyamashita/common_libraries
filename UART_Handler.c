...
extern uint8_t dmaArray[SIZE_OF_HPS_STRUCT];
...
int UART_TxMessage(UartTxMsgBufferStruct *msg, uint32_t ringPtr)
{
    ...
    uartPort = msg->BufStruct[0].uartPort;
    count = msg->BufStruct[0].dataSize;
    
    if(uartPort == UART_PORT_2) // Telemetry
    {
        UartTxMessageDMA(msg, ringPtr);
    }
    else
    {
        while(count){
            if(!(U1STAHbits.UTXBF == 1))
            {
                if(uartPort == 1)
                {
                    U1TXREG = msg->BufStruct[0].data[i];
                }
                else if(uartPort == 2)
                {
                    U2TXREG = msg->BufStruct[0].data[i];
                }
                count--;
                i++;
            }
...
int UartTxMessageDMA(UartTxMsgBufferStruct *msg, uint32_t ringPtr)
{
    ...
    memcpy(dmaArray, msg, count);
    
    DMA_LoadData(uartPort, dmaArray, count);
