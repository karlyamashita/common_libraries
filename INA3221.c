...
/*
 * Description: Converts mV value to hex value.
 * Input: the lsb resolution, 0.00004V or 0.080V, the voltage in mV
 * Output: the Hex value of the voltage.
 * Return: error status;
 *
 */
int INA3221_VoltageToHex(float resolution, float voltage, uint16_t *wholeNumber)
{
    uint8_t isNeg = 0;
    ...
    *wholeNumber = voltage / (resolution * 1000);

    *wholeNumber = *wholeNumber << 3;

    *wholeNumber = (~ *wholeNumber) & 0x7FFF;
...
/*
 * Description: Converts a hex value from the INA3221 register to a mV value
 * Input: The lsb resolution, 0.00004V or 0.080V, the voltage in mV
 * Output: the mV value
 * Return: error status
 *
...
int INA3221_HexToVoltage(float resolution, uint16_t data, double *retValue)
{
    ...
    decimalnumber = (uint16_t)data >> 3;

    if(isNeg)
    {
        *retValue = (decimalnumber * (resolution * 1000)) * -1;
    }
    else
    {
        *retValue = (decimalnumber * (resolution * 1000));
    }
